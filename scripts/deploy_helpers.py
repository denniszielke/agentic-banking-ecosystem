"""Shared helpers for building and deploying the banking MCP servers.

A trimmed-down version of the sample deploy helpers, focused solely on what the
MCP-server deployment scripts need: reading configuration from the environment,
building an image in Azure Container Registry (ACR) and deploying it as a
Container App via ``infra/core/host/app.bicep``.
...
def resolve_registry() -> str:
    ...
    discovery from the resource group. The azd outputs can be blank when hosted
    agents were not enabled, so discovery keeps deployment working as long as an
    ACR exists in the resource group.
    """
...
def build_image(
    ...
    if dockerfile:
        # Pass the Dockerfile as a path relative to the build context so
        # az acr build resolves the .dockerignore from the context root.
