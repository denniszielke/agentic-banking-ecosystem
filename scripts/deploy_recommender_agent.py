"""Deploy the **Recommender Agent** as an Azure Container App.

The Volksbank personal banking assistant — an AG-UI web agent that reaches
live account and transaction data through Microsoft Fabric data agents and
offers financial calculations via the Finance MCP server.

Unlike the compliance and employee agents (Foundry hosted agents), this one runs
as a public Container App serving the chat UI on port 8091.

Run it after ``azd up`` and after the search indexes are created and ingested.
The Fabric data agent and its Foundry project connection must exist before
deployment.

Usage::

    python -m scripts.deploy_recommender_agent --build   # build in ACR, then deploy
    python -m scripts.deploy_recommender_agent           # deploy existing image
...
Environment variables (populated from ``.env`` by ``azd up``):
  ...
  FABRIC_CONNECTION_ID                   Foundry project connection ID for the
                                         Fabric data agent (required)
  FINANCE_TOOLBOX_NAME                   finance MCP toolbox name (default: finance-tools)
  ...
  RECOMMENDER_AGENT_PORT                 container port (default: 8091)
  RECOMMENDER_AGENT_EXTERNAL             "true" for public ingress (default: true)
  COMPLIANCE_A2A_ENABLED                 "true" to consume the Compliance agent over A2A
...
IMAGE_NAME = "recommender-agent"
PORT = int(os.getenv("RECOMMENDER_AGENT_PORT", "8091"))
_DOCKERFILE = "src/recommender_agent/Dockerfile"
...
def assign_identity_roles(principal_id: str) -> None:
    ...
    _grant_role(principal_id, _COGNITIVE_SERVICES_USER, rg_scope)
    _grant_role(principal_id, _SEARCH_INDEX_DATA_READER, rg_scope)
    _grant_role(principal_id, _MONITORING_METRICS_PUBLISHER, rg_scope)
...
def deploy(tag: str | None = None) -> None:
    ...
    external = os.getenv("RECOMMENDER_AGENT_EXTERNAL", "true").strip().lower() == "true"
    model = (
    ...
    env_vars = {
        ...
        "AZURE_AI_COMPLIANCE_AGENT_NAME": os.getenv(
            "AZURE_AI_COMPLIANCE_AGENT_NAME", "compliance-agent"
        ),
        "COMPLIANCE_AGENT_A2A_URL": os.getenv("COMPLIANCE_AGENT_A2A_URL", ""),
        "COMPLIANCE_AGENT_AUDIENCE": os.getenv(
    ...
    if fqdn:
        print(f"\nRecommender Agent deployed: https://{fqdn}/")
    else:
