"""AG-UI server for the Recommender Agent (Volksbank personal banking assistant).

Hosts the recommender agent behind the AG-UI protocol (Server-Sent Events) and
serves a streaming single-page banking web UI from the same container.
...
Environment: see ``recommender_agent.py``. Additionally honours ``HOST``
and ``PORT`` (default 0.0.0.0:8091).
"""
...
from src.recommender_agent.recommender_agent import (  # noqa: E402
    _MODEL,
    _PROJECT_ENDPOINT,
    SYSTEM_PROMPT,
...
# Disable OTel instrumentation (same workaround as fabric_agent) unless
# explicitly opted in — the Fabric preview tool is not JSON-serialisable.
...
class CustomerProfile(BaseModel):
    ...
    bank: Optional[str] = Field(default=None, description="Bank name, e.g. 'Volksbank'.")
...
class AccountRow(BaseModel):
    ...
    product_name: str = Field(description="Product held, e.g. 'GiroKonto', 'VR-BankCard'.")
...
class PendingAction(BaseModel):
    kind: Optional[str] = Field(
        default=None, description="e.g. 'order_product', 'cancel_product', 'update_customer'."
    )
    summary: Optional[str] = Field(
        default=None, description="One-line description of the pending change."
    )
...
# ---------------------------------------------------------------------------
# Agent + context providers + tools (built once, kept alive for app lifetime)
# ---------------------------------------------------------------------------

_credential = DefaultAzureCredential()
_product_provider, _embedding_client = make_providers(_credential)
_fabric_tools = make_fabric_tools(_credential)
_finance_mcp_tool = make_finance_mcp_tool(_credential)
_compliance_tool, _compliance_a2a_agent = make_compliance_a2a_tool(_credential)

_extra_tools = []
if _finance_mcp_tool is not None:
    _extra_tools.append(_finance_mcp_tool)
if _compliance_tool is not None:
    _extra_tools.append(_compliance_tool)
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    async with AsyncExitStack() as stack:
        ...
        if _finance_mcp_tool is not None:
            close = getattr(_finance_mcp_tool, "__aexit__", None)
        ...
        logger.info("Recommender Agent ready (model=%s).", _MODEL)
        try:
            yield
        finally:
...
app = FastAPI(title="Recommender Agent — AG-UI", lifespan=lifespan)
...
def main() -> None:
    ...
    logger.info(
        "Starting Recommender Agent AG-UI server on http://%s:%d", host, port
    )
